...
const client = new Client({ authStrategy: new LocalAuth({ dataPath: '.' }), puppeteer: { args: ['--no-sandbox'] } });
const app = express();
...
function loadDatabase() { try { if (fs.existsSync(DB_FILE)) { const data = fs.readFileSync(DB_FILE, 'utf-8'); db = data.length > 0 ? JSON.parse(data) : {}; } else { fs.writeFileSync(DB_FILE, JSON.stringify({})); db = {}; } } catch (error) { customLog(`Gagal memuat database: ${error.message}`); db = {}; } }
function saveDatabase() { try { fs.writeFileSync(DB_FILE, JSON.stringify(db, null, 2)); } catch (error) { customLog(`Gagal menyimpan database: ${error.message}`); } }
function loadViolations() { try { if (fs.existsSync(VIOLATIONS_FILE)) { const data = fs.readFileSync(VIOLATIONS_FILE, 'utf-8'); violations = data.length > 0 ? JSON.parse(data) : []; } else { fs.writeFileSync(VIOLATIONS_FILE, '[]'); violations = []; } } catch (error) { customLog(`Gagal memuat violations: ${error.message}`); violations = []; } }
function saveViolations() { try { fs.writeFileSync(VIOLATIONS_FILE, JSON.stringify(violations, null, 2)); } catch (error) { customLog(`Gagal menyimpan violations: ${error.message}`); } }
function getUser(userId) { if (!db[userId]) { db[userId] = { nickname: userId.split('@')[0], role: 'user', isBanned: false, hasSeenRules: false }; saveDatabase(); } return db[userId]; }
function generateRoomId() { return `wafa${Math.floor(100000 + Math.random() * 900000)}`; }
function checkProfanity(message) { const words = message.toLowerCase().split(/\s+/); return badWords.some(word => words.includes(word)); }
function logChatMessage(roomId, userId, message, mediaType = 'text') { const logPath = `${LOG_DIR}/${roomId}.json`; const user = getUser(userId); const timestamp = new Date().toLocaleString('id-ID', { timeZone: 'Asia/Jakarta' }); const logEntry = { timestamp, userId, nickname: user.nickname, message: mediaType === 'text' ? message : `[Media: ${mediaType}]` }; try { let logs = []; if (fs.existsSync(logPath)) { logs = JSON.parse(fs.readFileSync(logPath, 'utf-8')); } logs.push(logEntry); fs.writeFileSync(logPath, JSON.stringify(logs, null, 2)); } catch (error) { customLog(`Gagal menulis log chat untuk room ${roomId}: ${error.message}`); } }
...
app.get('/api/chatlog/:roomId', (req, res) => {
    const roomId = req.params.roomId.replace(/[^a-zA-Z0-9]/g, ''); const logPath = `${LOG_DIR}/${roomId}.json`;
    try { if (fs.existsSync(logPath)) { const data = fs.readFileSync(logPath, 'utf-8'); res.json(JSON.parse(data)); } else { res.status(404).json({ error: 'Log tidak ditemukan' }); } } catch (error) { res.status(500).json({ error: 'Gagal membaca log' }); }
});
app.post('/toggle-ban', (req, res) => { loadDatabase(); const { userId } = req.body; const user = getUser(userId); if (user) { user.isBanned = !user.isBanned; saveDatabase(); } res.redirect('/dashboard'); });
app.post('/broadcast', (req, res) => {
    const { message } = req.body; if (!message) return res.status(400).send("Pesan tidak boleh kosong."); if (broadcastStatus.isRunning) return res.status(400).send("Broadcast lain sedang berjalan.");
    loadDatabase(); const allUserIds = Object.keys(db).filter(id => { const user = db[id]; return user && !user.isBanned && user.role !== 'admin'; });
    broadcastStatus = { isRunning: true, progress: 0, total: allUserIds.length, currentUser: '' };
    res.status(200).send("Broadcast dimulai!");
    (async () => {
        for (let i = 0; i < allUserIds.length; i++) {
            const userId = allUserIds[i]; const user = getUser(userId);
            broadcastStatus.currentUser = user.nickname; broadcastStatus.progress = i + 1;
            try { await client.sendMessage(userId, message); } catch (error) { customLog(`[BROADCAST] Gagal mengirim ke ${userId}: ${error.message}`); }
            const randomDelay = Math.floor(Math.random() * 8000) + 3000; await new Promise(resolve => setTimeout(resolve, randomDelay));
        }
        broadcastStatus.isRunning = false; broadcastStatus.currentUser = 'Selesai';
    })();
});
...
client.on('message', async (message) => {
    ...
    if (activeChats[user_id]) {
        ...
        if (lowerCaseText === '!stop' || lowerCaseText === '!skip') {
            ...
            await message.reply('Sesi chat diakhiri. Ketik *!chat* untuk mencari orang baru.');
            await client.sendMessage(partner_id, 'Dia telah meninggalkanmu. Ketik *!chat* untuk mencari orang baru.');
            return;
        }
        if (lowerCaseText === '!lapor') {
            ...
            let chatHistory = []; const logPath = `${LOG_DIR}/${roomId}.json`;
            try { if (fs.existsSync(logPath)) { const logContent = fs.readFileSync(logPath, 'utf-8'); chatHistory = JSON.parse(logContent).slice(-10); } } catch (error) { customLog(`Gagal membaca log untuk laporan: ${error.message}`); }
            ...
            await client.sendMessage(partner_id, 'Sesi chat dihentikan oleh sistem karena ada laporan dari partner.');
            return;
        }
        if (message.hasMedia && (message.type === 'image' || message.type === 'sticker' || message.type === 'video')) {
            ...
            try {
                ...
                const sendOptions = { caption: message.type === 'image' ? text : undefined, sendMediaAsSticker: message.type === 'sticker' || message.type === 'video' };
        ...
        if (checkProfanity(text)) {
            ...
            saveViolations();
            return message.reply('Gaboleh kasar woi!!. _autoreply_');
        }
        if (text) {
    ...
    const knownCommands = ['!chat', '!stop', '!stiker', '!stikergif', '!menu', 'halo', 'p', 'assalamualaikum'];
        ...
        switch (command) {
            ...
            case '!chat':
                ...
                if (waitingQueue.length > 0) {
                    ...
                    if (partner_id === user_id) { waitingQueue.push(user_id); return message.reply('Ups, hampir dapat diri sendiri. Mencari partner lain...'); }
            ...
            case '!stiker':
                ...
                } else { message.reply('Kirim gambarnya dulu dengan caption *!stiker* ya.'); }
                break;
            case '!stikergif':
                if (message.hasMedia && message.type === 'video') {
                    message.reply('🎥 Oke, stiker geraknya lagi diproses! Mohon tunggu...');
                    ...
                    try {
                        ...
                        exec(ffmpegCommand, async (error) => {
                            if (error) {
                                ...
                                message.reply('Duh, gagal membuat stiker gerak. Coba video lain yang lebih pendek.');
                            } else {
                                await client.sendMessage(message.from, MessageMedia.fromFilePath(tempOutputPath), { sendMediaAsSticker: true, stickerAuthor: "AnonyChat Bot", stickerName: `Animasi by ${user.nickname}` });
                            }
                    ...
                    message.reply('Kirim video/GIF dengan caption *!stikergif* ya (durasi maks 7 detik).');
                }
                break;
        }
    }
