...
function saveDatabase() {
    try { fs.writeFileSync(DB_FILE, JSON.stringify(db, null, 2)); } catch (error) { console.error('Gagal menyimpan database:', error); }
}
...
function saveViolations() {
    try { fs.writeFileSync(VIOLATIONS_FILE, JSON.stringify(violations, null, 2)); } catch (error) { console.error('Gagal menyimpan violations:', error); }
}
...
// PERBAIKAN: Fungsi log diubah untuk bisa mencatat tipe media
function logChatMessage(roomId, userId, message, mediaType = 'text') {
    ...
    const logEntry = { 
        ...
        message: mediaType === 'text' ? message : `[Media: ${mediaType}]` // Catat tipe media jika bukan teks
    };
    try {
        let logs = [];
...
client.on('message', async (message) => {
    ...
    if (activeChats[user_id]) {
        ...
        // Prioritaskan perintah !stop dan !lapor
        ...
        // --- PENAMBAHAN FITUR: MENERUSKAN MEDIA (FOTO & STIKER) ---
        if (message.hasMedia && (message.type === 'image' || message.type === 'sticker')) {
            ...
            try {
                ...
                logChatMessage(roomId, user_id, '', message.type); // Catat di log bahwa media dikirim
                await client.sendMessage(partner_id, media, { 
                    caption: message.type === 'image' ? text : undefined // Kirim caption hanya jika itu gambar
                });
            } catch (error) {
                console.error("Gagal meneruskan media:", error);
                message.reply('Duh, maaf, gagal meneruskan media.');
            }
            return; // Hentikan proses setelah media dikirim
        }
        
        // Jika bukan media atau perintah, teruskan sebagai teks biasa
